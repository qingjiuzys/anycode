...
- **Stdio line mode** (non-TTY `repl` / bare `anycode`): while a turn is running, a **`tokio::signal::ctrl_c`** side task sets the same cooperative flag; cancelled turns print the same **`tui-turn-cooperative-cancelled`** line as stream/TUI. A second interrupt may still kill the process depending on the OS.
