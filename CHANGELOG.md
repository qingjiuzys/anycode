...
- **`CronCreate`:** reject invalid cron expressions with field-count and parse hints; unsupported `schedule_timezone` values (only `local` / `utc` / `utc0`; IANA names return a clear error).
