...
- **Agent** / **Task** `run_in_background: true`: spawn nested `AgentRuntime` via `tokio::spawn`; **`TaskOutput`** exposes `background_status` / `background_summary`; **`TaskStop`** on the same UUID sets a cooperative cancel flag polled at nested **turn** and **tool** boundaries (shared `Arc<AtomicBool>`), with **`AbortHandle`** still as hard fallback. Cooperative exit does not interrupt an in-flight LLM request; syscall-blocking tools remain best-effort. **`config.json` `lsp`** section for **`tools-lsp`** (command, `workspace_root`, `read_timeout_ms`).
