...
|------|------------------|---------------------|
| 架构 | 单进程 ratatui + App 状态机 | **默认备用屏全屏** ratatui 轴心；从属 Tokio `select!` 承载回合与斜杠；**Inline 遗留** 下仍走宿主 **`insert_before`** |
| 主缓冲滚动 | 项目自述强调 native scrollback 等 | 遗留 Inline：`build_stream_turn_plain` → **`StreamReplRenderMsg`** → UI `insert_before`；默认全屏仅矩阵内 `transcript` |
