...
The client sends `tools` / `tool_choice` in **OpenAI Chat Completions** shape and parses native `tool_calls` (and legacy `function_call`) from responses. Parsing lives in `crates/llm/src/tool_call_normalizer.rs` on the shared OpenAI-compatible path (`ZaiClient` / optional `OpenAIClient`). Non-native textual/XML tool protocols are **not** supported — use a provider that returns structured `tool_calls`.

Managed **MiniCPM5-1B** runs via SGLang with native `tool_calls` when the sidecar is running.
