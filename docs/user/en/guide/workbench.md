...
- **Deliverable cards:** images, video, PDF, Office files, and mind maps from Skills/tools appear as cards in the thread — see [Conversation deliverables](./deliverables.md).
...
### Automated browser testing (Workbench)

1. **Prerequisites:** macOS/Windows/Linux with Chromium — desktop `.app` bundle, or Google Chrome on PATH, or `ANYCODE_CHROMIUM_PATH`.
2. **Enable:** Settings → Notifications → **Built-in browser** → Enable (writes `browser.enabled` in `~/.anycode/config.json`). Start a **new conversation** after toggling.
3. **Right panel:** Conversations → pick a **project-bound** session → right rail **Browser** (globe) → enter a URL to preview (live screencast).
4. **Agent automation:** In the same thread, prompt e.g. “打开 http://127.0.0.1:43180 并用 BrowserSnapshot 检查标题”. Tools: `BrowserNavigate`, `BrowserSnapshot`, `BrowserClick`, `BrowserType`, … — bound to the conversation id; panel and agent share one CDP session.
5. **Troubleshooting:** Doctor → `browser_connector`; panel shows setup steps if Chromium is missing. Dev-only `anycode-dashboard-serve` must be built with `--features tools-browser`.
