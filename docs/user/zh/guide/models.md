...
- `runtime.max_agent_turns` / `runtime.max_tool_calls`（可选）：限制单次任务的 LLM 往返轮数（默认 **8**）与累计工具调用次数（默认 **32**）。超出工具上限任务失败；超出轮数则以 summary 收尾。可在 Workbench **设置 → Agents** 编辑，或设环境变量 **`ANYCODE_MAX_AGENT_TURNS`** / **`ANYCODE_MAX_TOOL_CALLS`**（环境变量优先于配置）。
...
客户端按 **OpenAI Chat Completions** 形态下发 `tools` / `tool_choice`，并解析响应中的 `tool_calls`。

### 自动工具调用（OpenAI 兼容端点）

客户端按 **OpenAI Chat Completions** 形态下发 `tools` / `tool_choice`，并解析响应中的原生 `tool_calls`（及 legacy `function_call`）。解析逻辑在 `crates/llm/src/tool_call_normalizer.rs`（`ZaiClient` / 可选 `OpenAIClient`）。**不支持**把工具意图写在纯文本/XML 里的非标准协议 — 请使用能返回结构化 `tool_calls` 的端点。

托管 **MiniCPM5-1B** 通过 SGLang 侧车提供原生 `tool_calls`；侧car 运行后 Dashboard 不再显示「仅聊天预览」提示。
