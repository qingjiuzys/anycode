...
2. 在用户可见入口就绪时于 **`crates/bootstrap` 组合根**接线。

## 记忆后端与 pipeline

- **file / hybrid / noop**：由 `anycode-bootstrap` 的 `build_memory_layer` 配置（`crates/bootstrap/src/memory_setup.rs`）。
...
| 运行时组装 | `crates/bootstrap/src/runtime.rs` |
