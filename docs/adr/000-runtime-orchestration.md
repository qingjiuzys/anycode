...
anyCode exposes an **`Agent`** trait in `anycode-core` with an **`execute`** method, while the CLI, TUI, and daemon actually run tasks through **`AgentRuntime::execute_task`** and **`execute_turn_from_messages`**. Contributors can assume the wrong entry point unless the rule is documented.
...
2. **`Agent` trait role**: Supplies **agent type**, **tool name subset** (`tools()`), **description**, and **system prompt** hooks. The default **`Agent::execute`** implementations are **not** invoked by the current CLI/TUI/daemon main paths.
