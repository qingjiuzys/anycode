...
微信 daemon 在 **`SessionState::Processing`** 下收到新的非斜杠消息时会先置位当前回合的 **`nested_cancel`** 再 **abort** `execute_task`，并提示用户已中断（Fluent **`wx-interrupt-new-msg`**）。会话内 **`y` / `n` 审批** 由 `PermissionBroker` + `WaitingPermission` 状态处理。编排路径仍是整轮 **`execute_task`**；微信侧**不**推送 `🔧 tool` / `✓ tool` 等工具中间行，用户仅见最终回复。与 REPL **`execute_turn_from_messages` + 流式 sink** 的 HUD 仍不等价，细节见 [`wx-streaming-bridge.md`](wx-streaming-bridge.md)。

## 定时任务（Cron，与 OpenClaw 编排对齐）

- **工具**：`CronCreate` / `CronDelete` / `CronList`（`anycode-tools`，落盘 **`~/.anycode/tasks/orchestration.json`**）。
- **执行**：`anycode scheduler`（`crates/cli/src/scheduler.rs`）读取同一 JSON，按计划触发与 `anycode run` 等价的单次任务。

## Digital Workbench（Dashboard）与 Desktop

**单一 `anycode` 二进制、多运行模式**（详见 [`run-flow.md`](run-flow.md)）：

- **`anycode dashboard`**：Axum HTTP 服务（默认 `127.0.0.1:43180`），SQLite `~/.anycode/projects.db`，嵌入或挂载 `dashboard-ui` 静态资源。
- **Agent 不在 dashboard 进程内执行**：UI 通过 `task_trigger` spawn `anycode run` 子进程；`DashboardRecorder` tail `output.log` 写入 DB；审批/取消经文件 IPC（`approval_ipc` / `cancel_ipc`）。
- **Desktop（Tauri）**：`apps/anycode-desktop` 仅 spawn `anycode dashboard` sidecar + WebView，不直接调用 `initialize_runtime`。
...
- **单实例**：**`~/.anycode/tasks/scheduler.lock`**（独占锁）保证同机只有一个调度循环。**WeChat / Telegram / Discord** 长驻桥与独立 **`anycode scheduler`** **抢同一把锁**；任一桥在启动时 **`tokio::spawn`** 内置调度器的尝试抢锁失败时静默退出嵌入（聊天仍可用，但须有**另一进程**持锁才能使 cron 落火）。
