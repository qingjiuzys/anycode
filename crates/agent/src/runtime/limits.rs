...
//! 可配置上限见 [`anycode_core::AgentLoopLimits`] / `config.json` → `runtime`.
...
mod tests {
    use super::*;
    use anycode_core::{DEFAULT_MAX_AGENT_TURNS, DEFAULT_MAX_TOOL_CALLS};

    #[test]
    fn limits_are_positive() {
        assert!(DEFAULT_MAX_AGENT_TURNS > 0);
        assert!(DEFAULT_MAX_TOOL_CALLS >= DEFAULT_MAX_AGENT_TURNS);
