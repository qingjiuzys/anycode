    ...
    let summary_messages = vec![
        Message {
            ...
            content: MessageContent::Text(
                ...
                版式要求：第一行必须是 `### 完成回执`；正文使用 `**已完成：**`、`**关键步骤：**` 三段式；若有失败或未通过门禁，追加 `**失败原因：**`。使用规范 Markdown；加粗必须写成 `**词语**`，`**` 与文字之间不要空格；不要在连续汉字之间插入空格（英文单词之间照常保留空格）。若使用 `###` 小标题，编号连续且同一标题只出现一次，不要重复粘贴上一节内容。".to_string()
            ),
