...
import { formatExecutionLogLine } from "@/lib/eventFormat";
    ...
    <SectionCard title={t("session.executionLog")}>
      ...
      {lines.length > 0 && (
        <ul className="m-0 p-0 list-none space-y-2 font-code text-xs">
          {lines.map((line) => {
            const formatted = formatExecutionLogLine(line, t);
            return (
            <li
            ...
            >
              <div className="flex flex-wrap items-center gap-2 text-secondary mb-1">
                ...
                {formatted.eventType && (
                  <span className="text-primary">{formatted.eventType}</span>
                )}
                {formatted.title && <span>{formatted.title}</span>}
              </div>
              {line.body ? (
