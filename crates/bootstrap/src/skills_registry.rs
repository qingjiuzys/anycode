...
pub async fn fetch_extra_skill_roots(url: &str) -> Vec<PathBuf> {
    let url = url.trim();
    if url.is_empty() {
        return Vec::new();
    }
    ...
    let Ok(resp) = client.get(url).send().await else {
        tracing::warn!(target: "anycode_bootstrap", "skills.registry_url: request failed");
        return Vec::new();
    };
    let Ok(text) = resp.text().await else {
        tracing::warn!(target: "anycode_bootstrap", "skills.registry_url: empty body");
        return Vec::new();
    };
    let Some(paths) = parse_manifest_extra_scan_roots(&text) else {
        tracing::warn!(target: "anycode_bootstrap", "skills.registry_url: invalid JSON manifest");
        return Vec::new();
    };
