...
fn parse_mcp_servers_json(raw: &str) -> Vec<McpServerEntry> {
    let v: Value = match serde_json::from_str(raw) {
        ...
        Err(e) => {
            tracing::warn!(
                ...
                error = %e,
                "skipping invalid MCP servers JSON"
            );
            return vec![];
        }
    };
    let Some(arr) = v.as_array() else {
        tracing::warn!(target: "anycode_bootstrap", "MCP servers JSON must be an array");
        return vec![];
    };
    let mut out = Vec::new();
    for (i, item) in arr.iter().cloned().enumerate() {
        ...
        if cmd.is_empty() {
            tracing::warn!(target: "anycode_bootstrap", index = i, "skipping MCP entry without command");
            continue;
        }
        let slug = obj
            .get("slug")
