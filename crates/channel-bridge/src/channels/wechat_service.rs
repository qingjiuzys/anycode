...
//! Installed after `anycode-daemon bridge wechat` bind success; service uses hidden `--run-as-bridge`.
...
fn macos_env_plist_block(spec: &WechatServiceSpec) -> String {
    ...
    if let Some(helper) = anycode_apple_media::resolve_apple_media_helper(&[]) {
        pairs.push((
...
/// After `anycode-daemon bridge wechat` scan success: install autostart for current exe + data root.
