use crate::app_config::{apply_channel_self_hosted_security, Config};
...
/// `anycode-daemon bridge telegram-set-token`：写入凭据，供后续无 `--bot-token` 启动。
...
pub(crate) async fn run_telegram_polling(mut config: Config, args: TelegramRunArgs) -> Result<()> {
    ...
    .into_arc();
    let project_enabled = anycode_bootstrap::load_project_enabled_skills(&workdir).await;
...
pub(crate) async fn run_telegram_setup() -> Result<()> {
    ...
    println!("使用 `anycode-daemon bridge telegram` 启动轮询桥。");
    Ok(())
}
