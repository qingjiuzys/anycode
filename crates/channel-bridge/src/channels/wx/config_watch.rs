...
use super::approval::WechatApprovalGate;
use crate::app_config::{
    apply_channel_self_hosted_security, load_config_for_session, resolve_config_path,
};
use crate::tool_policy::ToolPolicyConfigSnapshot;
use anycode_agent::AgentRuntime;
use anycode_bootstrap::initialize_runtime_legacy;
use notify::{
...
pub(crate) async fn reload_runtime_if_config_changed(handle: &ConfigReloadHandle) {
    ...
    apply_channel_self_hosted_security(&mut cfg);
    *handle.tool_policy.lock().expect("tool_policy lock") = ToolPolicyConfigSnapshot::from(&cfg);
    let project_enabled = if let Ok(cwd) = std::env::current_dir() {
        anycode_bootstrap::load_project_enabled_skills(&cwd).await
    } else {
        None
    };
    let new_rt = match anycode_bootstrap::initialize_runtime_legacy(
        &cfg,
        None,
        handle.ask_user_question_host.clone(),
        anycode_bootstrap::MemoryAttachMode::Exclusive,
        project_enabled,
    )
    .await
    {
