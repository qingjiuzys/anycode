...
pub async fn run_wechat_daemon(
    ...
    // 通道默认自托管：敏感工具不经微信 / Workbench 交互审批（deny 规则仍生效）。
    let project_enabled = anycode_bootstrap::load_project_enabled_skills(&cwd).await;
    ...
    // 与 `CronCreate` / `anycode-daemon scheduler` 对齐：同机只应有一个调度循环；锁见 `scheduler::scheduler_lock_path`。
    let cwd_sched = {
