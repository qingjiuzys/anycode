...
wizard-no-config-model = No config at {$path}. Run `Workbench Settings → Models` or `~/.anycode/config.json` first.
wizard-run-config-first = Run: ~/.anycode/config.json
wizard-model-empty = model must not be empty.
wizard-unknown-model = Unknown model: {$id}. Available: {$list}
wizard-provider-not-supported = Provider {$p} does not support `Workbench Settings → Models set` (use anthropic/claude or z.ai)
...
cfg-next-example-cmd =   Workbench chat --agent general-purpose "your prompt"
cfg-wechat-hint-non-tty = To bind WeChat and install autostart bridge: anycode-daemon wechat-bridge
...
cfg-no-config-run =    Run: Workbench /setup — or open the Digital Workbench (/setup) to configure in the browser
...
err-provider = Invalid provider: {$p}. Run `Workbench Settings → Models` for ids or see docs.
...
err-memory-backend = Invalid memory.backend: {$b} (allowed: noop, none, off, file, hybrid, pipeline, layered, guigen)
...
log-channel-self-hosted-no-approval = Channel (WeChat / Telegram / Discord): self-hosted mode — tool approval disabled for this process (require_approval not written to disk)
...
err-github-copilot-token = For GitHub Copilot as global provider, set api_key (GitHub PAT) or run `Workbench Settings → Models auth copilot`
...
log-model-instructions-fail = ⚠️ Failed to load model_instructions_file {$path}: {$err}
