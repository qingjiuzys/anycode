...
wizard-no-config-model = 未检测到 {$path}，请先运行 Workbench Settings → Models 或 ~/.anycode/config.json
wizard-run-config-first = 先运行：~/.anycode/config.json
wizard-model-empty = model 不能为空。
wizard-unknown-model = 未知 model：{$id}。可用模型：{$list}
wizard-provider-not-supported = 当前 provider={$p} 不支持 Workbench Settings → Models set（请用 anthropic/claude 或 z.ai）
...
cfg-next-example-cmd =   Workbench chat --agent general-purpose "用中文帮我分析这个项目"
cfg-wechat-hint-non-tty = 提示：绑定微信并安装登录自启后台桥可运行：anycode-daemon wechat-bridge
cfg-wechat-confirm = 是否现在绑定微信并安装登录自启后台桥？
cfg-skip-wechat = 已跳过微信绑定（--skip-wechat）。
cfg-no-config-warn = ⚠️  Warning: 未检测到 {$path}
cfg-no-config-run =    运行 Workbench /setup，或在可视化工作台打开 /setup 完成配置
...
err-provider = 无效的 provider：{$p}。请使用 Workbench Settings → Models 列出目录中的厂商 id，或查阅文档
err-unknown-zai-model = 未知 model：{$id}。可用模型：{$list}
err-no-home-memory = 无法解析主目录（HOME 未设置），无法解析 memory.path 默认路径
err-memory-backend = 无效的 memory.backend：{$b}（允许 noop、none、off、file、hybrid、pipeline、layered、guigen）
...
log-channel-self-hosted-no-approval = 通道（微信 / Telegram / Discord）：自托管模式 — 本进程关闭工具交互审批（配置中的 require_approval 未写回磁盘）
...
err-github-copilot-token = GitHub Copilot 为全局 provider 时请在 api_key 填入 GitHub PAT，或先运行 `Workbench Settings → Models auth copilot`
