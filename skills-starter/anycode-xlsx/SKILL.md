...
## Artifact emit（run 脚本自动）

`run` 结束时 stdout 输出：

```
ANYCODE_ARTIFACT:{"path":"/abs/out.xlsx","kind":"spreadsheet","title":"out.xlsx","preview_path":"/abs/workbook.preview.html","inline":true}
```

并写入 `out.xlsx.anycode-artifact.json`。
