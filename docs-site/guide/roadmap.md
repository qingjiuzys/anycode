...
- **Agent / legacy Task:** nested runs use the same **`AgentRuntime`** (`SubAgentExecutor`); **`agent_type`** selects **explore** / **plan** / **general-purpose** tool surfaces; nesting depth is capped. Tool results include **`nested_task_id`** (runtime UUID) for correlation with **`~/.anycode/tasks/<id>/output.log`**. **`TaskOutput`** accepts that UUID and returns **`output_log_path`** plus a tail of the log when present; **`TaskCreate` / `Task*`** orchestration records (plus teams, crons, etc.) **persist** to **`~/.anycode/tasks/orchestration.json`** — a separate id namespace from execution UUIDs. Further work: isolation, permission inheritance, and optional linking from orchestration records to execution ids (e.g. via metadata).
