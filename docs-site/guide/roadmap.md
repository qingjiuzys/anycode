...
|--------|-----------------------------|
| **Nested-task cooperative cancel (v2)** | **Partial (shipped):** shared cancel flag from **`TaskStop`** through nested **`TaskContext`**, polled each **turn** and around **tool** calls; **`TaskResult::Failure`** with error **`cancelled`** maps to **`background_status: cancelled`**. **Still open:** bind cancel to **streaming / blocking LLM** requests; optional **`tokio_util::sync::CancellationToken`** for `select!` ergonomics. **Syscall-blocked** tools remain **best-effort** (**`AbortHandle`**). |
