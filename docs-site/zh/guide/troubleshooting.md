...
**`setup` 中途失败？**  
多半是网络或 API 密钥问题。检查模型服务商的密钥是否填写正确，然后重新运行 `Workbench /setup`。

## 工作台

**浏览器打不开 `http://127.0.0.1:43180`？**  
先在终端执行 anyCode desktop or Workbench at http://127.0.0.1:43180，看是否提示端口被占用。若被占用，可在设置里改端口后重启。

**项目列表是空的？**  
工作台只显示**已经跑过任务**的工作区。请在工作台项目会话中发起对话，再刷新页面。
...
**打开应用但页面空白？**  
等几秒让内置服务启动；仍不行则在终端手动执行 anyCode desktop or Workbench at http://127.0.0.1:43180 后再打开应用。
